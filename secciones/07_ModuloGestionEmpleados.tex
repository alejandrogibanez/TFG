...
La capacidad para dar de alta a un nuevo empleado está restringida a los usuarios que dispongan de permisos de recursos humanos (“rrhh”).
El flujo de trabajo en la interfaz está diseñado de forma secuencial: una vez que el usuario autorizado escoge
una empresa en la plataforma mediante el menú de selección, aparecerá habilitado el botón de “dar de alta”, simbolizado en la esquina superior derecha con el icono de un ``+'' (más).
Al pulsar este botón, el sistema abrirá una nueva pestaña o vista desplegando el formulario de empleados,
donde se deberán introducir todos los datos requeridos del nuevo trabajador.
...
Tras esta edición de información, se envía al endpoint de la empresa la información actualizada del empleado, lo que a su vez puede desencadenar una nueva comunicación con la TGSS para reflejar los cambios en el sistema de la Seguridad Social, dependiendo de la naturaleza de las modificaciones realizadas (por ejemplo, cambio de domicilio, modificación de condiciones contractuales, etc.).
...
Como podemos observar en la figura \ref{fig:edicion_empleado}, el sistema muestra un formulario pre-rellenado con los datos actuales del empleado. Esto se consigue al haber una búsqueda en la base de datos para encontrar los datos del empleado y plasmarlos en dicho formulario, permitiendo al usuario autorizado realizar las modificaciones necesarias. Al confirmar los cambios, se actualiza la información tanto en la plataforma como en la TGSS, asegurando que todos los registros estén sincronizados y reflejen la situación actual del trabajador.
